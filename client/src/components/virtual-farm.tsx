...
export default function VirtualFarm({ farm, progress }: VirtualFarmProps) {
  ...
  const safeCrops = farm?.primaryCrops && farm.primaryCrops.length > 0 ? farm.primaryCrops : ['Mixed Crop'];
  const safeBadges = progress?.badges || []; // 👈 This fixes the crash!
  const safeVillage = farm?.village || 'Village';
  const safeDistrict = farm?.district || 'District';
  const safeState = farm?.state || 'State';
    ...
    <Card className="overflow-hidden" data-testid="virtual-farm">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          ...
          <Badge className={`${getSustainabilityColor(safeScore)} text-white`}>
            Healthy
          </Badge>
        </div>
      </CardHeader>
      ...
      <div className="farm-gradient h-48 relative">
        <div className="absolute inset-0 p-4 flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 h-24">
            {safeCrops.slice(0, 2).map((crop, index) => {
          ...
          <div className="flex justify-between items-end">
            <div className="flex space-x-2">
              {safeBadges.includes('Compost Master') && (
              ...
              {safeBadges.includes('Water Saver') && (
      ...
      <CardContent className="p-4 bg-muted/50">
        <div className="flex justify-between text-sm">
          <div className="flex items-center" data-testid="farm-location">
            ...
            <span>{safeVillage}, {safeDistrict}, {safeState}</span>
          </div>
